...
function ItemsList({ data }) {
	...
	const messaging = useContext(ContextType.MESSAGING);
	...
	const deleteActionHandle = async (id) => {
	...
	const actions = [
		{
			...
			action: ({ row }) => {
				navigator.push(RootRoute.ITEM, { id: row.dataItem.id });
			},
		},
		{
			label: 'Edit',
			...
			action: ({ row }) => {
				navigator.push(RootRoute.ITEM_EDIT, { id: row.dataItem.id });
			},
		},
		{
			label: 'Delete',
	...
	const columns = useMemo(() => [
		{ name: 'name', label: 'Name', binding: 'name', type: DataGrid.ColumnType.TEXT_BOX, stretchFactor: 1 },
		{ name: 'description', label: 'Description', binding: 'custrecord_pcpart_description', type: DataGrid.ColumnType.TEXT_BOX, stretchFactor: 1 },
		{ name: 'category', label: 'Category', binding: 'custrecord_pcpart_category_name', type: DataGrid.ColumnType.TEXT_BOX, stretchFactor: 1 },
		{ name: 'vendor', label: 'Vendor', binding: 'vendorLink', type: DataGrid.ColumnType.LINK },
		{ name: 'price', label: 'Price', binding: 'custrecord_pcpart_price', type: DataGrid.ColumnType.TEXT_BOX, stretchFactor: 1 },
		{ name: 'instock', label: 'In Stock', binding: 'inStock', type: DataGrid.ColumnType.CHECK_BOX },
		{ name: 'action', label: 'Action', type: DataGrid.ColumnType.ACTION, actions },
	], [actions]);

	const textFilterRef = useRef();
	const [textFilterValue, setTextFilterValue] = useState('');
	const [sortedDataSource, setSortedDataSource] = useState(parsedData);
	...
	const filteredDataSource = useMemo(() => {
		...
		return sortedDataSource.filter(({ name }) =>
			Type.EmptyString.is(textFilterValue) ? true : name.toLowerCase().includes(textFilterPhrase)
		);
