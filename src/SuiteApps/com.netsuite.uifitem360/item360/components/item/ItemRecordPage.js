import { ContextType, Navigator, SystemIcon, useContext, useDispatch, UserMessageService } from '@uif-js/core';
import { ApplicationHeader, ContentPanel, Loader, ScrollPanel, StackPanel, Text } from '@uif-js/component';
import * as ItemRecord from '../../data/ItemRecord.js';
...
export default function ItemRecordPage({ record, saving, mode }) {
	...
	if (record === null) {
		return (
			<ContentPanel>
				<Text>Invalid Record ID</Text>
			</ContentPanel>
		);
	} else {
		const saveEditMode = async (id) => {
			...
			<StackPanel.Vertical loader={loader}>
				...
				<StackPanel.Item grow={1}>
					<ScrollPanel orientation={ScrollPanel.Orientation.VERTICAL}>
						<RecordForm record={record} readonly={readonly} formScheme={ItemRecord.FormSchemeObject} />
					</ScrollPanel>
				</StackPanel.Item>
			</StackPanel.Vertical>
		);
