import { ArrayDataSource, Date, useMemo, useState } from '@uif-js/core';
...
export default function RecordField({ record, fieldId, readonly }) {
...
function RecordFieldControl({ record, field, readonly, validSetter, statusMessageSetter, valid }) {
	switch (field.type) {
...
function RecordFieldTextBox({ record, readonly, field, validSetter, statusMessageSetter, valid }) {
...
function RecordFieldCurrency({ record, readonly, field, validSetter, statusMessageSetter, valid }) {
	const { id, isReadOnly, isDisabled } = field;
	const text = record.getText(id);
...
function RecordFieldTextArea({ record, readonly, field, validSetter, statusMessageSetter, valid }) {
	const { id, isReadOnly, isDisabled } = field;
	const text = record.getText(id);
...
function RecordFieldCheck({ record, readonly, field }) {
	const { id, isReadOnly, isDisabled } = field;
...
function RecordFieldSelect({ record, readonly, field, validSetter, statusMessageSetter, valid }) {
	const { id, isReadOnly, isDisabled } = field;
...
function RecordFieldDate({ record, readonly, field, validSetter, statusMessageSetter, valid }) {
	const { id, isReadOnly, isDisabled } = field;
